...
void initialize_mpi(int argc, char* argv[], int& rank, int& size, int& K, int& num_threads) {
    MPI_Init(&argc, &argv);
    ...
    if (argc != 3) {
        if (rank == 0) {
            std::cerr << "Usage: " << argv[0] << " <number_of_clusters> <number_of_openmp_threads>" << std::endl;
        }
        MPI_Finalize();
        exit(1);
    }
    ...
    num_threads = atoi(argv[2]); // Number of OpenMP threads

    if (K <= 0 || num_threads <= 0) {
        if (rank == 0) {
            std::cerr << "Number of clusters and threads must be positive integers." << std::endl;
        }
        MPI_Finalize();
        exit(1);
    }
...
#pragma omp parallel
    {
        std::vector<Point> private_sums(K, { 0.0, 0.0 });
        std::vector<int> private_counts(K, 0);

#pragma omp for nowait
        for (int i = 0; i < static_cast<int>(local_points.size()); ++i) {
            int cluster_id = local_assignments[i];
            private_sums[cluster_id].first += local_points[i].first;
            private_sums[cluster_id].second += local_points[i].second;
...
#pragma omp critical
        {
            for (int i = 0; i < K; ++i) {
                local_sums[i].first += private_sums[i].first;
                local_sums[i].second += private_sums[i].second;
...
int main(int argc, char* argv[]) {
    int rank, size, K, num_threads;
    initialize_mpi(argc, argv, rank, size, K, num_threads);
